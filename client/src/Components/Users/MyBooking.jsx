  ...
  useEffect(() => {
    ...
    async function fetchBooking() {
      try {
        ...
        const currentUserBooking = response?.data?.bookings?.filter((booking) => booking?.organizer?._id === user?._id);
              ...
              <div className="border-l-4 border-l-[#ED4A43] p-6">
                ...
                <div className="mt-4 flex justify-end gap-2">
                  {booking.status === "approved" && (
                    <button className="px-4 py-2 text-sm bg-[#ED4A43] text-white rounded hover:bg-red-600" onClick={() => handleStripePayment(booking._id)}>
                      Pay Now
                    </button>
                  )}
                </div>
